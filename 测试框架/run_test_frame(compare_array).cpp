...
void run_tests(const string &test_file)
{
    ...
    while (getline(infile, line))
    {
        ...
        vector<int> expected_sorted_array(n);
        for (int i = 0; i < n; ++i)
        {
        ...
        //     correct_tests++;
        // }
        if (sorted_array.size() == expected_sorted_array.size() && equal(sorted_array.begin(), sorted_array.end(), expected_sorted_array.begin()))
        {
            correct_tests++;
        }
